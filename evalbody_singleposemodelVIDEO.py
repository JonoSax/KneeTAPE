...
def jointFinder(vidPath, outputPath):

    print("\nVideo: " + vidPath + "\n")

    vidName = vidPath.split("/")[-1].split(".")[0]
    ...
    f = open(outputPath + vidName +'jointmovement.txt', 'w')
    ...
    print(vidPath + " mask done")
    
    # print("\n ----- Video making -----")
    cap = cv2.VideoCapture(vidPath)
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter(outputPath + vidName + "segmented.mp4", fourcc, 30, (frame.shape[1], frame.shape[0]))  # use previous frame size
    if (cap.isOpened()== False): 
        print(vidPath + " Error opening video stream or file")
    n = 0
    while(cap.isOpened()):
...
if __name__ == "__main__":
    ...
    if serialised:
        for v in vidPaths:
            jointFinder(v, outputPath)

    else:
        with multiprocessing.Pool(processes=6) as pool:
            pool.starmap(jointFinder, zip(vidPaths, repeat(outputPath)))
